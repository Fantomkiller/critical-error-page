...
Istniejący workflow `.github/workflows/pages.yml` publikuje także statyczny podgląd GitHub Pages po pushu. Buduje go z prefiksem `/critical-error-page/`; formularz wysyła zgłoszenia do Workera obecnej publicznej strony Sites, z ograniczonym CORS dla `https://fantomkiller.github.io`. To pozwala korzystać z działającej rekrutacji na Pages jeszcze przed konfiguracją Cloudflare. Github Pages i Sites to dwa osobne adresy: automatyczna aktualizacja M+ z harmonogramu pojawi się na Pages po commicie danych, a na Sites dopiero po ręcznym wdrożeniu tej wersji lub migracji na Cloudflare.
