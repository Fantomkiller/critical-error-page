...
export function MythicPlus({ players, apiUrl, configReady, assetsBase = '/' }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!Array.isArray(data.players)) throw new Error('Invalid M+ ranking');
  ...
  const source = ranking?.players || (apiUrl ? [] : players);
  ...
  const scope = apiUrl ? 'GILDIA' : 'DRUŻYNA MAIN';
